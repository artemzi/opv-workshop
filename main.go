...
var (
	stdlog = log.New(os.Stdout, "[WORKSHOP]: ", log.LstdFlags)
	errlog = log.New(os.Stderr, "[WORKSHOP:ERROR]: ", log.Ldate|log.Ltime|log.Lshortfile)
)
...
func root(c *router.Control) {
	c.Code(http.StatusOK).Body(fmt.Sprintf("WORKSHOP v%s\n", version.RELEASE))
}
